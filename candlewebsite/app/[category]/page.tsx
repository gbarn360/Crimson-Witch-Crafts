...
export default function page({ params }: { params: { category: string } }) {
    ...
    useEffect(() =>{
        ...
        axios.get("http://localhost:4000/items").then(response => console.log(response.data.filter((item:Item)=>item.category === category)));
            ...
            <div className="my-10 mx-auto pt-5  md:w-5/6 ">
                ...
                <div className='flex flex-wrap'>
                    {category.map((item:Item, index:number) => (
                        <Link key={index} href={"#"} className='p-5 sm:w-1/2 md:w-1/3 2xl:w-1/4'>
